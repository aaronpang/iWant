//
//  Constants.m
//  iWant
//
//  Created by Aaron Pang on 2014-04-27.
//  Copyright (c) 2014 Aaron Pang. All rights reserved.
//

#import "Constants.h"

@implementation Constants

NSString * const IWFontName = @"HelveticaNeue-Light";
NSString * const IWItalicFontName = @"HelveticaNeue-LightItalic";
NSString * const IWDogeFontName = @"MarkerFelt-Thin";

NSString * const IWDefaultSearchTerm = @"restaurants";

const NSInteger IWMetersPerMile = 1609.344;
const NSInteger IWDogeFontSize = 30;
const NSInteger IWFontSize = 55;
const NSInteger IWGoButtonSize = 120;
const NSInteger IWCancelButtonSize = 80;
const NSInteger IWResultButtonSize = 86;
const CGFloat IWTimeoutTime = 60.0f;

NSString * const IWStopQuestionErrorDomain = @"IWStopQuestionErrorDomain";

@end
